...
# cu_loc = c(40.6892, -74.0445) #statue of liberty
...
ui <- fluidPage(
  ...
  div(
    id = "loading-content",
    img(src = "cover.png", height = 5375/7.465, width = 3492/7.465)
  ),
           ...
           # hidden(
             ...
             class = "outer",
               ...
               absolutePanel(id = "controls", class = "panel panel-default", fixed = TRUE,
                           ...
                           selectInput(inputId = "day", label = h3(" Today is"), 
                           ...
                           # checkboxInput(inputId = "gpsfrom", label = "Use Real-time Location", value = FALSE),
                           
                           #input destination
                           textInput(inputId = "end",label = "To: ", value = ""),
                           
                           # checkboxInput(inputId = "gpsto", label = "Use Real-time Location", value = FALSE),
               ...
               # absolutePanel(id = "fare_board", class = "panel panel-default", fixed = TRUE,
               #               draggable = TRUE,top = "auto", left = 0, right = "auto", bottom = 200,
             ...
             # absolutePanel(id = "user_interactive", class = "panel panel-default", fixed = TRUE,
             #               draggable = FALSE,top = 30, left = 0, right = "auto", bottom = "auto",
             #               width = 320, height = "auto",
             #               
             #               selectInput(inputId = "day", label = "Today is", choices = c("Monday", "Tuesday"), selected = "Monday")
             # )
           )
           # )
  )
...
server<-function(input, output, session){
  ...
  # })
  # 
  # output$rtloc = renderText({
  #   cu.loc()
  # })
  
  # observeEvent(input$gpsfrom, {
  #   js$geoloc()
  # })
  # 
  # observe({
  #   ## localize myself
  #   cu_loc = c(input$lat, input$long)
  #   output$rtloc = renderText({
  #     cu_loc
  #   })
  # })
  ...
  output$fares = renderText({
    ...
    if (hour(t())==0 & minute(t()) <5){
      prev_5min = na.omit(per()%>%subset(tpep_dropoff_datetime>=(t()-30*60)+24*60*60 | tpep_dropoff_datetime<=t()))
    } else{
      #get previous 5 minutes data for fare estimation
      prev_5min = na.omit(per()%>%subset(tpep_dropoff_datetime>=(t()-30*60) & tpep_dropoff_datetime<=t()))
    }
