...
# Functions
get_route <- function(pickupAddress, dropoffAddress, mode ='driving', key = 'AIzaSyAXvkJsyLxLxWLcnImVsMnOoiZJjbcIBYA'){
  
  #origin = as.numeric(as.vector(t(geocode(pickupAddress, source = "dsk"))))
  #destination = as.numeric(as.vector(t(geocode(dropoffAddress, source = "dsk"))))
  origin = c(pickupAddress[1], pickupAddress[2])
  destination = c(dropoffAddress[1],dropoffAddress[2])
  
  doc <- mp_directions(origin, destination ,mode,alternatives = TRUE,
                       key = key)
