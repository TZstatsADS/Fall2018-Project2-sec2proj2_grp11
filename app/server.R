...
# source("lib/geosphere_demo.R")
# external functions
calc_geo_distance2 = function(long1, lat1, long2, lat2, units = "km") {
  # calc the distance for single route
  loadNamespace("purrr")
...
#     
    ...
    output$map3 <- renderLeaflet({
      leaflet() %>%
        setView(lat=40.808091, lng=-73.959813, zoom=12) %>%
        addTiles() 
    })
    ...
    showTaxisPopup <- function(current_time, click_event){
      ...
      temp_taxi_data <- test01 %>% 
        subset( (abs(pickup_longitude-lng) < 0.0001 
                                           & abs(pickup_latitude - lat) < 0.0001) ) %>%
              subset( (abs(minute(hm(pickup_time) - hm(time))) <= 15)  | 
                  (abs(minute(hm(dropoff_time) - hm(time))) <= 15) )
      # | (abs(dropoff_longitude-lng) < 0.0001 
      #    & abs(dropoff_latitude - lat) < 0.0001 )
      # 
      count1 <- nrow(temp_taxi_data)
      print(as.character(paste("count1", count1, sep = ":")))
      ...
      if (count1 > 5) {
        ...
        content = "a car"
        for(i in nrow(temp_taxi_data)){
          leafletProxy("map3") %>% addPopups(lat = temp_taxi_data$pickup_latitude, 
                                             lng = temp_taxi_data$pickup_longitude, 
                                             popup = content) 
          # %>% addMarkers(lat = temp_taxi_data$pickup_latitude, 
          #             lng = temp_taxi_data$pickup_longitude, icon = greenLeafIcon)
        }
        
      } else {
        temp_taxi_data <- test01 %>% 
          subset( (abs(pickup_longitude-lng) < 0.001 
                   & abs(pickup_latitude - lat) < 0.001) ) %>%
          subset( (abs(minute(hm(pickup_time) - hm(time))) <= 20)  | 
                    (abs(minute(hm(dropoff_time) - hm(time))) <= 20) )
        # | (abs(dropoff_longitude-lng) < 0.0001 
        #    & abs(dropoff_latitude - lat) < 0.0001 )
        # 
        count2 <- nrow(temp_taxi_data)
        print(count1)
        # pop up generation
      }
